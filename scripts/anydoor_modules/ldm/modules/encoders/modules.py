...
import open_clip
from scripts.anydoor_modules.ldm.util import default, count_params
...
from scripts.anydoor_modules.dinov2 import hubconf
from omegaconf import OmegaConf
from modules.devices import device
config_dir = os.path.join(scripts.basedir(), "configs")
model_dir = os.path.join(scripts.basedir(), "models")
config = OmegaConf.load(os.path.join(config_dir, "anydoor.yaml"))
DINOv2_weight_path = os.path.join(model_dir, config.model.params.cond_stage_config.weight)

class FrozenDinoV2Encoder(AbstractEncoder):
    """
    Uses the DINOv2 encoder for image
    """
    def __init__(self, device=device, freeze=True):
        super().__init__()
